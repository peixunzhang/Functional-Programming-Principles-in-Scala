...
trait Huffman extends HuffmanInterface {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    def merge(fo: Fork, trees: List[CodeTree]): List[CodeTree] = trees match {
      case Nil => List(fo)
      case x :: xs =>
        if (weight(fo) < weight(x)) fo :: trees
        else x :: merge(fo, xs)
    }
    trees match {
      case x :: y :: xs =>
        val ct = makeCodeTree(x, y)
        merge(ct, xs)
      case xs => xs
    }
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def go(subtree: CodeTree, bits: List[Bit]): List[Char] =
      (subtree, bits) match {
        case (Leaf(char, _), Nil)            => List(char)
        case (Leaf(char, _), xs)             => char :: go(tree, xs)
        case (Fork(_, right, _, _), 1 :: xs) => go(right, xs)
        case (Fork(left, _, _, _), 0 :: xs)  => go(left, xs)
        case _                               => ???
      }
    go(tree, bits)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encodeOne(tree: CodeTree, a: Char): List[Int] = tree match {
      case Fork(left, right, _, weight) =>
        if (chars(left).contains(a)) 0 :: encodeOne(left, a)
        else 1 :: encodeOne(right, a)
      case Leaf(char, weight) if (char == a) => Nil
  ...
  def convert(tree: CodeTree): CodeTable = tree match {
    case Leaf(char, weight) => Map((char, Nil))
    case Fork(left, right, chars, weight) =>
      val newLeft = convert(left).view.mapValues(0 :: _)
      val newRight = convert(right).view.mapValues(1 :: _)
      mergeCodeTables(newRight.toMap, newLeft.toMap)
  }
